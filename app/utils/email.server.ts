...
export async function sendEmail({
	...
	if (!process.env.MAIL_SERVICE_API_KEY && !process.env.MOCKS) {
		console.error(`MAIL_SERVICE_API_KEY not set and we're not in mocks mode.`)
		console.error(`To send emails, set the MAIL_SERVICE_API_KEY environment variable.`)
	...
	const response = await fetch(`${process.env.MAIL_SERVICE_API_ENDPOINT}/emails`, {
		method: 'POST',

...
export async function signup({ email, username, password, name }: { email: User['email']; username: User['username']; name: User['name']; password: string }) {
	const hashedPassword = await getPasswordHash(password)
...
export async function logout(
	...
	throw redirect(safeRedirect(redirectTo), {
		...responseInit,
		headers: combineHeaders({ 'set-cookie': await authSessionStorage.destroySession(authSession) }, responseInit?.headers),
	})
}

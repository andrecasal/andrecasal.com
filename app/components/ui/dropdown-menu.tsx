...
>(({ className, inset, children, ...props }, ref) => (
	<DropdownMenuPrimitive.SubTrigger
		ref={ref}
		className={cn('flex select-none items-center rounded-sm px-2 py-1.5 text-size-sm outline-none focus:bg-muted-100 data-[state=open]:bg-muted-100', inset && 'pl-8', className)}
		{...props}
	>
		{children}
...
>(({ className, inset, ...props }, ref) => (
	<DropdownMenuPrimitive.Item
		ref={ref}
		className={cn(
			'relative flex select-none items-center rounded-sm px-2 py-1.5 text-size-sm outline-none transition-colors focus:bg-muted-100 focus:text-muted-900 data-[disabled]:pointer-events-none data-[disabled]:opacity-50',
			inset && 'pl-8',
			className,
		)}
		{...props}
...
const DropdownMenuCheckboxItem = React.forwardRef<React.ElementRef<typeof DropdownMenuPrimitive.CheckboxItem>, React.ComponentPropsWithoutRef<typeof DropdownMenuPrimitive.CheckboxItem>>(
	({ className, children, checked, ...props }, ref) => (
		<DropdownMenuPrimitive.CheckboxItem
			ref={ref}
			className={cn(
				'relative flex select-none items-center rounded-sm py-1.5 pl-8 pr-2 text-size-sm outline-none transition-colors focus:bg-muted-100 focus:text-muted-900 data-[disabled]:pointer-events-none data-[disabled]:opacity-50',
				className,
			)}
			checked={checked}
			{...props}
...
>(({ className, inset, ...props }, ref) => <DropdownMenuPrimitive.Label ref={ref} className={cn('px-2 py-1.5 text-size-sm font-semibold', inset && 'pl-8', className)} {...props} />)
...
const DropdownMenuShortcut = ({ className, ...props }: React.HTMLAttributes<HTMLSpanElement>) => {
	return <span className={cn('ml-auto text-size-xs tracking-widest opacity-60', className)} {...props} />
}

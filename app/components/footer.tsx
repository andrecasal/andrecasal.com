							...
							<ul className="flex flex-wrap gap-x-8 gap-y-2">
								{secondaryNav.map(item => (
									<li key={item.name}>
										<Span size="sm">
											{item.external ? (
												<a href={item.href} target="_blank" rel="noreferrer" className="text-muted-600 hover:text-muted-900">
													{item.name}
												</a>
											) : (
												<Link to={item.href} className="text-muted-600 hover:text-muted-900">
													{item.name}
												</Link>
											)}
										</Span>

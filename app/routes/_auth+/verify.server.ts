...
import { handleVerification as handleChangeEmailVerification } from '~/routes/settings+/profile.change-email.server.tsx'
import { handleVerification as handleLoginTwoFactorVerification, shouldRequestTwoFA } from './login.server.ts'
...
import { VerifySchema, codeQueryParam, redirectToQueryParam, targetQueryParam, typeQueryParam, type VerificationTypes } from './verify.tsx'
...
export function getRedirectToUrl({ request, type, target, redirectTo }: { request: Request; type: VerificationTypes; target: string; redirectTo?: string }) {
...
export async function prepareVerification({ period, request, type, target }: { period: number; request: Request; type: VerificationTypes; target: string }) {
...
export async function isCodeValid({ code, type, target }: { code: string; type: VerificationTypes | typeof twoFAVerifyVerificationType; target: string }) {
